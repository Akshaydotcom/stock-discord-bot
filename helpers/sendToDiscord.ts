...
export function buildDiscordEmbed(stockData: StockData, shares: number) {
  const isPositive = stockData.change > 0;
  ...
  const embeds: object[] = [
    {
      title: `${emoji} ${stockData.symbol} Stock Update`,
      url: `https://finance.yahoo.com/quote/${stockData.symbol}`,
      description: `Current stock price for ${stockData.symbol}`,
      fields: [
        { name: 'Current Price', value: `$${stockData.price.toFixed(2)}`, inline: true },
        { name: 'Change', value: `$${stockData.change.toFixed(2)}`, inline: true },
        { name: 'Change %', value: `${stockData.changePercent.toFixed(2)}%`, inline: true },
        { name: 'Volume', value: stockData.volume.toLocaleString(), inline: false },
      ],
      color,
      timestamp: new Date().toISOString(),
  ...
  // Only add portfolio embed if shares are provided
  if (shares > 0) {
    embeds.push({
      title: '💼 Your Portfolio',
      description: 'Updated portfolio value',
      fields: [
        { name: 'Shares Owned', value: `${shares}`, inline: true },
        { name: 'Portfolio Value', value: `$${(shares * stockData.price).toFixed(2)}`, inline: true },
        { name: "Today's P&L", value: `$${(shares * stockData.change).toFixed(2)}`, inline: true }
      ],
      color,
      timestamp: new Date().toISOString()
    });
